		...
		**
		...
		drop T
			...
			**
			...
			expand 	2 if affected   == 1, gen(REP)
			
			replace affected  		=  2 if REP == 1
			replace network 		=  0 if REP == 1
			...
			**			
			egen 	total_enrollment 	 = rsum(enrollmentEI enrollmentEF1 enrollmentEF2 enrollmentEMtotal)
			egen 	total_school 		 = rsum(school_EI school_EF1 school_EF2 school_EM)

			replace total_enrollment     = . if total_enrollment    == 0
			replace total_school 	 	 = . if total_school 		== 0 
			
			su 		total_enrollment if affected == 2
			local   enrollment_affected = r(mean)
			
			su 		total_enrollment if affected == 0
			local 	enrollment_unaffected = r(mean)
			
			su 		total_school 	 if affected == 2
			local   schools_affected = r(mean)
			
			su 		total_school 	 if affected == 0
			local 	schools_unaffected = r(mean)
			
			local 	share_enrollment  	= (`enrollment_affected'/(`enrollment_affected' + `enrollment_unaffected'))*100
			local 	share_schools 		= (`schools_affected'/(`schools_affected' + `schools_unaffected'))*100
			...
			**
			...
			replace network2 = "Extended winter-break"   					in 5
			replace network2 = "Did not extended the winter break" 			in 6
			replace network2 = "Total"										in 4
			...
			**
			replace total_enrollment	= "Total"				in 7
			replace total_school	    = "Total"				in 7 
			replace enrollmentEI  		= "Pre-K" 				in 7
			replace enrollmentEF1 		= "Elementary"  		in 7
			replace enrollmentEF2 		= "Lower sec" 			in 7
			replace enrollmentEMtotal 	= "High School"  		in 7
			replace school_EI  			= "Pre-K"				in 7
			replace school_EF1 			= "Elementary"  		in 7
			replace school_EF2 			= "Lower sec"  			in 7
			replace school_EM 			= "High School"  		in 7
			
			order 	network2 total_enrollment enrollment* total_school school*
			
			replace total_enrollment = "`share_enrollment'" in 8 
			replace total_school     = "`share_schools'"    in 8 
			
			replace total_enrollment = substr(total_enrollment,1,4) in 8
			replace total_school	 = substr(total_school,1,4)     in 8 
			...
			**
			...
			replace order = 6 in 6
			replace order = 7 in 1
			
			replace order = 8 in 8
		...
		*--------------------->>
		{
			...
			**
				...
				use "$final/h1n1-school-closures-sp-2009.dta" if year == 2009 & math5 !=.,  clear
					...
					*
					replace 	G0_est = "Municipalities in which the local" 				in 7
					...
					**
					export excel "$tables/Table2.xlsx", replace
		}
	}
	...
	**
	**
	*Tables A1: OLS, 2007 at municipalily level, probability of closing schools according to  characteristics of the municipalities
	**
	*--------------------------------------------------------------------------------------------------------------------------------*
	{
		**
		...
		**
		...
		estout * using "$tables/TableA1.xls",  label varlabels(_cons Constant) cells(b(star fmt(3))  se(fmt(3))) stats(N r2_a, labels ("Obs" "Adj. R-squared") fmt(%9.5g %9.4f) ) replace
	}
	...
	**
	**
	*Tables A2, A3, A4, A5: balance test
	**
	*--------------------------------------------------------------------------------------------------------------------------------*
	...
	{	
		use "$final/h1n1-school-closures-sp-2009.dta" if year == 2009 & !missing(math5) & uf == "SP", clear
			...
			*
				iebaltab  $balance_students5  $matching_schools school_year enrollmentTotal pib_pcap if network == 3 & sample_dif == 1			,  format(%12.2fc) grpvar(T) savetex("$tables/TableA2") 	rowvarlabels replace 
			...
			*	
				iebaltab  $balance_students5 $matching_schools  school_year enrollmentTotal pib_pcap if G == 1 		 & sample_triple_dif == 1	, format(%12.2fc) grpvar(E) savetex("$tables/TableA3") 	rowvarlabels replace 
			...
			*	
				iebaltab  $balance_students5 $matching_schools  school_year enrollmentTotal pib_pcap if G == 0 		 & sample_triple_dif == 1   , format(%12.2fc) grpvar(E) savetex("$tables/TableA4") 	rowvarlabels replace 
			...
			*	
				...
				iebaltab  $balance_students5 $matching_schools school_year enrollmentTotal pib_pcap if network == 2	 & sample_triple_dif == 1   , format(%12.2fc) grpvar(G)	savetex("$tables/TableA5") 	rowvarlabels replace 
	}
	
	
	**
	**
	*Tables A6, A7, A8, A9: balance test
	**
	*--------------------------------------------------------------------------------------------------------------------------------*
	{	
		...
		use "$final/h1n1-school-closures-sp-2009.dta" if uf == "SP", clear
			...
			*
				...
				iebaltab  $balance_teachers $balance_principals if year == 2009 & !missing(math5) & network == 3   & sample_dif == 1			, format(%12.2fc) grpvar(T) savetex("$tables/TableA6.tex")   rowvarlabels replace 
			...
			*
				...
				iebaltab  $balance_teachers $balance_principals if year == 2009 & !missing(math5) & G == 1  		& sample_triple_dif == 1	, format(%12.2fc) grpvar(E) savetex("$tables/TableA7")   rowvarlabels replace 
			...
			*
				...
				iebaltab  $balance_teachers $balance_principals if year == 2009 & !missing(math5) & G == 0 			& sample_triple_dif == 1	, format(%12.2fc) grpvar(E) savetex("$tables/TableA8")   rowvarlabels replace 
			...
			*
				...
				iebaltab  $balance_teachers $balance_principals if year == 2009 & !missing(math5) & network == 2	& sample_triple_dif == 1 , format(%12.2fc) grpvar(G) savetex("$tables/TableA9")  rowvarlabels replace 
	...
	**
	**
	*Table A10: 	Checking parallel trends of state and local-managed schools prior to 2009. Dependent variables: math and portuguese performance
	**
	*--------------------------------------------------------------------------------------------------------------------------------*
	{
	...
	estimates clear
		...
		*Regressions
		...
		estout * using "$tables/TableA10.xls", keep(beta*) label cells(b(star fmt(3))  se(fmt(2))) stats(N r2_a, fmt(%9.0g %9.3f)) replace
		...
		**Regressions in G = 1 and G = 0
		...
		estout * using "$tables/dif-dif-g1-g0.xls", keep(beta*) delimiter(";") label cells(b(star fmt(3))  se(fmt(2))) stats(N r2_a, fmt(%9.0g %9.3f)) replace
		*/
	}	
	...
	*--------------------------------------------------------------------------------------------------------------------------------*
	{
		...
		*Regressions
		...
		estout * using "$tables/TableA11.xls", keep(beta*) label cells(b(star fmt(3))  se(fmt(2))) stats(N r2_a, fmt(%9.0g %9.3f)) replace
	}	
